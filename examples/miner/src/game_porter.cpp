...
void Porter::CatchFlyingRocks() {
	...
	// range catch
	auto& g = scene->flyingRocksGrid;
	auto cri = g.PosToCRIndex(pos);
	auto rockRadius = scene->cRockRadius * scene->cRocksScale;
	auto r = catchRange + rockRadius;
	auto rr = r * r;
	g.ForeachByRange(cri.y, cri.x, catchRange + rockRadius * 3, gg.sgrdd, [&](xx::Grid2dCircle<FlyingRock*>::Node& node, float distance)->bool {
		auto o = node.value;
		auto d = o->pos - pos;
		auto mag2 = d.x * d.x + d.y * d.y;
		if (mag2 < rr) {
...
bool Porter::SearchTarget() {
	...
	if (scene->flyingRocks.len < 10) {
		for (auto& o : scene->flyingRocks) {
			auto d = o->pos - pos;
			auto mag2 = d.x * d.x + d.y * d.y;
			if (mag2 < minMag2) {
				minMag2 = mag2;
		...
		auto rr = r * r;
		int32_t limit{ 5 };
		g.ForeachByRange(cri.y, cri.x, searchRange + rockRadius * 3, gg.sgrdd, [&](xx::Grid2dCircle<FlyingRock*>::Node& node, float distance)->bool {
			auto o = node.value;
			auto d = o->pos - pos;
			auto mag2 = d.x * d.x + d.y * d.y;
			if (mag2 < rr) {
...
#else
	if (scene->flyingRocks.len > 0) {
		target = gg.rnd.NextElement(scene->flyingRocks);
	}
#endif
