...
namespace xx {
    ...
    struct BitmapDC {
        ...
        Data getTextureDataForText(std::string_view text, const FontDefinition& textDefinition, TextAlign align, int& width, int& height) {
            if (!setFont(textDefinition._fontName, (int)textDefinition._fontSize, false)) {
                CoutN("Can't found font( ", textDefinition._fontName, " ), use system default");
            }
...
inline std::pair<float, float> upload_unicode_char_to_texture(int charSize, char const* buf) {
    using namespace xx;
    int w{}, h{};
    auto d = BitmapDC::instance->getTextureDataForText(buf, { "Consolas", 24 }, TextAlign::LEFT, w, h);
    if (w > 0) {
        GLTexParameteri();
